...
namespace NopTop.Plugin.Payments.Zarinpal
{
    ...
    {
        public static string ProduceRedirectUrl(string StoreLocation, int? code, bool UseSandbox, string Authority, string ZarinGate)
        {
            string urlProduced = code.HasValue && code.Value == 100 ?
               string.Concat($"https://{(UseSandbox ? "sandbox" : "payment")}.zarinpal.com/pg/StartPay/", Authority, ZarinGate)
                               :
               string.Concat(StoreLocation, "Plugins/PaymentZarinpal/ErrorHandler", "?Error=", ZarinpalHelper.StatusToMessage(code).Message)
                                ;
        ...
        public static StatusToResult StatusToMessage(int? _status)
        {
            ...
            switch (_status)
            {
                ...
                case -42:
                    ...
                    break;
                case -51:
                    statusToResult.Message = "پرداخت ناموفق";
                    break;
                case -54:
